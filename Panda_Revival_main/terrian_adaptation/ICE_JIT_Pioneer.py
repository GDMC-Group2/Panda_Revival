...
class Pioneer:
    ...
    def __init__(self, height_map, area_with_border, mergeArea_meanHeight, s_x, s_z, gravity_pos):
        self.limit_num = 5
        begin_time = time()
        self.height_limit = 5
        # self.level = level
    ...
    def findAreaToLevelling(self):
        ...
        for i in range(len(direction_l)):
            ...
            while not self.done_flag:
                ...
                #     p_x = x + self.s_x
                #     p_z = z + self.s_z
                #     setBlock(p_x, self.height_map.getHeight(x, z), p_z, 'pink_wool')
            ...
            #     p_x = x + self.s_x
            #     p_z = z + self.s_z
            #     setBlock(p_x, self.height_map.getHeight(x, z), p_z, 'light_blue_wool')
        # print pos_l
        ...
        for x in range(self.x1, self.x2 + 1):
            for z in range(self.z1, self.z2 + 1):
                if self.area_with_border[x, z] == 3 and self.isBorder(x, z):
                    ...
                    p_x = x + self.s_x
                    p_z = z + self.s_z
                    # setBlock(p_x, self.height_map.getHeight(x, z), p_z, 'black_wool')
