...
def buildHouse(editor,x, y, z, t_id, w_id,r_id,q_id,p_id):
    def wall(editor,x,y,z):
        for yy in range(12):
            for xx in range(11):
                for zz in range(27):
...
def stairs1(editor,x,y,z,t_id,q_id,p_id):
    

    def stairs_l(editor,x,y,z):
        h=0
        for xx in range(20):
            h=h+1
            for zz in range(7):
    ...
    stairs_l(editor,x+18,y,z+1)
    stairs_r(editor,x-18,y,z+1)
    wall_l(editor,x+18,y,z+10)
    wall_r(editor,x-18,y,z+10)
    wall_n(editor,x,y+16,z)
def stairs2(editor,x,y,z,t_id,q_id,p_id):
    ...
    stairs_l(editor,x+18,y,z+17)
    stairs_r(editor,x-18,y,z+17)
    wall_l(editor,x+18,y,z+6)
    wall_r(editor,x-18,y,z+6)
    wall_n(editor,x,y+16,z)
def buildlou(editor,x,y,z,q_id,w_id,e_id):
    ...
    zhuzi_w(editor,x,y+1,z+7)
    zhuzi_n(editor,x,y+1,z-7)

def buildhen(editor,x,y,z,q_id,w_id,e_id):
    def zhuzi_w(editor,x,y,z):
    ...
    def wuding_1(editor,x,y,z):
        for xx in range(26):
            for zz in range(17):
                editor.placeBlock((x+xx,y,z+zz),Block(w_id))
    def wuding_2(editor,x,y,z):
        for xx in range(20):
            for zz in range(11):
                editor.placeBlock((x+xx,y,z+zz),Block(w_id))
    def wuding_3(editor,x,y,z):
        for xx in range(20):
            for zz in range(7):
                editor.placeBlock((x+xx,y,z+zz),Block(w_id))
    def wuding_4(editor,x,y,z):
        for xx in range(20):
            for zz in range(5):
                editor.placeBlock((x+xx,y,z+zz),Block(w_id))
    def wuding_5(editor,x,y,z):
        for xx in range(20):
            for zz in range(3):
                editor.placeBlock((x+xx,y,z+zz),Block(w_id))
    for xx in range(20):
    ...
    zhuzi_w(editor,x,y+14,z+7)
    zhuzi_n(editor,x,y+14,z-7)
    wuding_1(editor,x-13,y+15,z-8)
    wuding_2(editor,x-10,y+16,z-5)
    wuding_3(editor,x-10,y+17,z-3)
    wuding_4(editor,x-10,y+18,z-2)
    wuding_5(editor,x-10,y+19,z-1)

def buildwa(editor,x,y,z,q_id,w_id,e_id):
    def wa1_1(editor,x,y,z):
        for xx in range(10):
    ...
    def wa1_4(editor,x,y,z):
        for zz in range(8):
            for xx in range(4):
                editor.placeBlock((x+xx,y,z+2*zz),Block(q_id))
            for xx in range(2):
                editor.placeBlock((x+xx,y+1,z+2*zz),Block(w_id,{"type": "bottom"}))
    def wa2_4(editor,x,y,z):
        for zz in range(7):
            editor.placeBlock((x,y,z+2*zz),Block(q_id))
            editor.placeBlock((x+3,y-1,z+2*zz),Block(w_id,{"type": "top"}))
            for xx in range(2):
    ...
    wa1_1(editor,x-9,y,z-1)
    wa2_1(editor,x-10,y,z-1)
    wa1_2(editor,x-9,y,z+1)
    wa2_2(editor,x-10,y,z+1)
    wa1_3(editor,x-11,y-4,z-7)
    wa2_3(editor,x-11,y-4,z-6)
    wa1_4(editor,x+10,y-4,z-7)
    wa2_4(editor,x+10,y-4,z-6)

def buildfeiyan(editor,x,y,z,q_id,w_id,e_id,r_id):
    ...
    def xiaofeiyan4(editor,x,y,z):
        ...
        editor.placeBlock((x+3,y,z+3),Block(r_id,{"up": "true"}))
        editor.placeBlock((x+4,y-1,z+4),Block(r_id,{"up": "true"}))
    feiyan1(editor,x+8,y,z-1)
    feiyan2(editor,x+8,y,z+1)
    feiyan3(editor,x-9,y,z-1)
    feiyan4(editor,x-9,y,z+1)
    shou_1(editor,x-9,y+3,z)
    shou_2(editor,x+8,y+3,z)
    xiaofeiyan1(editor,x-13,y-10,z+9)
    xiaofeiyan2(editor,x-13,y-10,z-9)
    xiaofeiyan3(editor,x+13,y-10,z-9)
    xiaofeiyan4(editor,x+13,y-10,z+9)
def buildxiawuding(editor,x,y,z,q_id,w_id,e_id,r_id):
    def muliang_1(editor,x,y,z):
        for xx in range(23):
            for zz in range(2):
                editor.placeBlock((x+xx,y,z+zz),Block(q_id))
    def muliang_2(editor,x,y,z):
        for xx in range(23):
            for zz in range(2):
                editor.placeBlock((x+xx,y,z-zz),Block(q_id))
    def muliang_3(editor,x,y,z):
        for xx in range(2):
            for zz in range(12):
                editor.placeBlock((x+xx,y,z+zz),Block(q_id))
    def muliang_4(editor,x,y,z):
        for xx in range(2):
            for zz in range(12):
                editor.placeBlock((x-xx,y,z+zz),Block(q_id))

    def muliang2_1(editor,x,y,z):
        for xx in range(27):
            for zz in range(6):
                editor.placeBlock((x+xx,y,z+zz),Block(q_id))
    def muliang2_2(editor,x,y,z):
        for xx in range(27):
            for zz in range(6):
                editor.placeBlock((x+xx,y,z-zz),Block(q_id))
    def muliang2_3(editor,x,y,z):
        for xx in range(6):
            for zz in range(16):
                editor.placeBlock((x+xx,y,z+zz),Block(q_id))
    def muliang2_4(editor,x,y,z):
        for xx in range(6):
            for zz in range(16):
                editor.placeBlock((x-xx,y,z+zz),Block(q_id))

    def muliang3_1(editor,x,y,z):
        for xx in range(33):
            for zz in range(12):
                editor.placeBlock((x+xx,y,z+zz),Block(q_id))
    def muliang3_2(editor,x,y,z):
        for xx in range(33):
            for zz in range(12):
                editor.placeBlock((x+xx,y,z-zz),Block(q_id))
    def muliang3_3(editor,x,y,z):
        for xx in range(12):
            for zz in range(22):
                editor.placeBlock((x+xx,y,z+zz),Block(q_id))
    def muliang3_4(editor,x,y,z):
        for xx in range(12):
            for zz in range(22):
                editor.placeBlock((x-xx,y,z+zz),Block(q_id))
    def qiang(editor,x,y,z):
        for yy in range(2):
            for xx in range(19):
    ...
    muliang_1(editor,x-11,y+10,z-7)
    muliang_2(editor,x-11,y+10,z+7) 
    muliang_3(editor,x-11,y+10,z-5)
    muliang_4(editor,x+11,y+10,z-5)
    muliang2_1(editor,x-13,y+9,z-9)
    muliang2_2(editor,x-13,y+9,z+9) 
    muliang2_3(editor,x-13,y+9,z-7)
    muliang2_4(editor,x+13,y+9,z-7)
    muliang3_1(editor,x-16,y+8,z-12)
    muliang3_2(editor,x-16,y+8,z+12) 
    muliang3_3(editor,x-16,y+8,z-10)
    muliang3_4(editor,x+16,y+8,z-10)
    qiang(editor,x-9,y+11,z-6)
    wa1_1(editor,x-12,y+11,z-7)
    wa1_2(editor,x-11,y+11,z-7)
    wa2_1(editor,x-12,y+11,z+7)
    wa2_2(editor,x-11,y+11,z+7)
    wa3_1(editor,x+11,y+11,z-8)
    wa3_2(editor,x+11,y+11,z-7)
    wa4_1(editor,x-11,y+11,z-8)
    wa4_2(editor,x-11,y+11,z-7)
    feiyan1(editor,x+13,y+9,z+10)
    feiyan2(editor,x-13,y+9,z+10)
    feiyan3(editor,x-13,y+9,z-10)
    feiyan4(editor,x+13,y+9,z-10)
    feiyan5(editor,x+14,y+9,z-9)
    feiyan6(editor,x+14,y+9,z+9)
    feiyan7(editor,x-14,y+9,z+9)
    feiyan7(editor,x-14,y+9,z-9)

def buildzhuangshi(editor,x,y,z,q_id,w_id,e_id):
...
def wall_r1(editor,x,y,z,q_id,w_id):
    for yy in range(16):
        for xx in range(10):
            for zz in range(10):
...
def wall_r2(editor,x,y,z,q_id,w_id):
    for yy in range(16):
        for xx in range(10):
            for zz in range(10):
                 editor.placeBlock((x+xx,y+yy,z+zz),Block(q_id))
...
def wall_l1(editor,x,y,z,q_id,w_id):
    for yy in range(16):
        for xx in range(10):
            for zz in range(10):
                 editor.placeBlock((x+xx,y+yy,z+zz),Block(q_id))
...
def wall_l2(editor,x,y,z,q_id,w_id):
    for yy in range(16):
        for xx in range(10):
            for zz in range(10):
                 editor.placeBlock((x+xx,y+yy,z+zz),Block(q_id))
...
def wall_x(editor,x,y,z,l,q_id,w_id,e_id):
    for yy in range(16):
        for xx in range(l):
            for zz in range(11):
                 editor.placeBlock((x+xx,y+yy,z+zz),Block(q_id))
...
def wall_z(editor,x,y,z,l,q_id,w_id,e_id):
    for yy in range(16):
        for xx in range(11):
            for zz in range(l):
                 editor.placeBlock((x+xx,y+yy,z+zz),Block(q_id))
...
def wall1(editor,x,y,z):
    buildHouse(editor,x, y, z, "stone_bricks", "stripped_acacia_log","acacia_slab","stone_brick_stairs","stone_brick_slab")
    stairs1(editor,x,y,z,"stone_bricks","stone_brick_stairs","stone_brick_slab")
    buildlou(editor,x, y+16, z+14,"stone_bricks","infested_chiseled_stone_bricks","red_concrete")
    buildhen(editor,x, y+16, z+14,"red_concrete","stripped_acacia_wood","stone_bricks")
    buildwa(editor,x, y+36, z+14,"cobblestone","cobblestone_slab","cobblestone_stairs")
    buildfeiyan(editor,x, y+36, z+14,"stone_bricks","stone_brick_slab","stone_brick_stairs","cobblestone_wall")
    buildxiawuding(editor,x, y+16, z+14,"stripped_acacia_wood","cobblestone","cobblestone_slab","cobblestone_stairs")
    buildzhuangshi(editor,x, y+16, z+14,"lantern","stripped_acacia_wood","ladder")
    for xx in range(35):
        for zz in range(8):
            for yy in range(18):
...
def wall2(editor,x,y,z):      
    buildHouse(editor,x, y, z, "stone_bricks", "stripped_acacia_log","acacia_slab","stone_brick_stairs","stone_brick_slab")
    stairs2(editor,x,y,z,"stone_bricks","stone_brick_stairs","stone_brick_slab")
    buildlou(editor,x, y+16, z+14,"stone_bricks","infested_chiseled_stone_bricks","red_concrete")
    buildhen(editor,x, y+16, z+14,"red_concrete","stripped_acacia_wood","stone_bricks")
    buildwa(editor,x, y+36, z+14,"cobblestone","cobblestone_slab","cobblestone_stairs")
    buildfeiyan(editor,x, y+36, z+14,"stone_bricks","stone_brick_slab","stone_brick_stairs","cobblestone_wall")
...
def wall_R1(editor,x,y,z):
    wall_r1(editor,x,y,z,"stone_bricks","stone_brick_slab")
def wall_R2(editor,x,y,z):
    wall_r2(editor,x,y,z,"stone_bricks","stone_brick_slab")
def wall_L1(editor,x,y,z):
    wall_l1(editor,x,y,z,"stone_bricks","stone_brick_slab")
def wall_L2(editor,x,y,z):
    wall_l2(editor,x,y,z,"stone_bricks","stone_brick_slab")
def wall_X(editor,x,y,z,l):
    wall_x(editor,x,y,z,l,"stone_bricks","stone_brick_slab","torch")
def wall_Z(editor,x,y,z,l):
    wall_z(editor,x,y,z,l,"stone_bricks","stone_brick_slab","torch")


def make_wall(editor,height, area): # for example: buildarea = (0, 0, 300 ,300)
    buildarea = (area[0], area[1], area[2]+area[0], area[3]+area[1])
    wall2(editor,buildarea[0]+area[2]//2, height, buildarea[1]-6)
    wall_X(editor,buildarea[0]+area[2]//2+35, height, buildarea[1], (buildarea[0]+area[2]//2)-buildarea[0]-35-10)
    wall_X(editor,buildarea[0]+10, height, buildarea[1], (buildarea[0]+area[2]//2)-buildarea[0]-35-9)


    wall1(editor,buildarea[0]+area[2]//2, height, buildarea[3]-21)
    wall_X(editor,buildarea[0]+area[2]//2+35, height, buildarea[3]-11, (buildarea[0]+area[2]//2)-buildarea[0]-35-10)
    wall_X(editor,buildarea[0]+10, height, buildarea[3]-11, (buildarea[0]+area[2]//2)-buildarea[0]-35-9)

    wall_L1(editor,buildarea[2]-10, height, buildarea[3]-10)
    wall_L2(editor,buildarea[2]-10, height, buildarea[1])
    wall_R1(editor,buildarea[0], height, buildarea[3]-10)
    wall_R2(editor,buildarea[0], height, buildarea[1])
    wall_Z(editor,buildarea[0], height, buildarea[1]+10, buildarea[3]-buildarea[1]-20)
    wall_Z(editor,buildarea[2]-11, height, buildarea[1]+10, buildarea[3]-buildarea[1]-20)
